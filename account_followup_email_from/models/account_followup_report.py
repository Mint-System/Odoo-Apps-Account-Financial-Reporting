...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _send_email(self, options):
        ...
        for to_send_partner in self._get_email_recipients(options):
            ...
            if email and email.strip():
                ...
                attachment_ids = options.get(
                    "attachment_ids", partner._get_invoices_to_print(options).message_main_attachment_id.ids
                )
                # If the follow-up was executed manually,
                # the author_id will be set to the ID of the current logged-in user.
                # Otherwise, if the follow-up is automatic,
                # the author_id will be the followup responsible or OdooBot.
                author_id = options.get("author_id", partner._get_followup_responsible().partner_id.id)

                email_from_parameter = (
                    self.env["ir.config_parameter"].sudo().get_param("account_followup_email_from.email_from", "")
                )
                ...
                partner.with_context(
                    mail_post_autofollow=True, mail_notify_author=True, lang=partner.lang or self.env.user.lang
                ).message_post(
        ...
        if not sent_at_least_once:
            raise UserError(
                _(
                    "You are trying to send an Email, but no follow-up contact "
                    " has any email address set for customer '%s'",
                    partner.name,
                )
            )
