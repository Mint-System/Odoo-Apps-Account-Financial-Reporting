...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_column_details(self, options):
        ...
        do_not_show_columns = [self._field_column('expected_pay_date'), self._field_column('account_name'), self._field_column('report_date')]
        ...
        columns_without_account_and_date = [column for column in columns if not column.name in do_not_show_names]

        # Insert new column header for Invoice Date
        columns_without_account_and_date[1:1] = [self._field_column('move_invoice_date')]
